...
export const logoutUser = asynchandler(async(req,res,next)=>{
    ...
    return res.clearCookie("token").status(200).json(new ApiResponse(200, "User logged out successfully"))
})
...
export const requestPasswordToken = asynchandler(async(req,res, next)=>{
    ...
    try{
        ...
        return res.status(200).json(new ApiResponse(200, "Reset email sent successfully"));
        ...
        await user.save({validateBeforeSave:false});
        return next(new ApiError(500, "Email could not be sent please try again later", err))
    }
})
...
export const resetPassword = asynchandler(async(req,res, next)=>{
   const token = req.params.token;
    ...
    if(password!==confirmPassword){
        return next(new ApiError(401, "password and confirmPassword doesnot match"))
    }
...
export const getUserDetails = asynchandler(async(req,res,next)=>{
    ...
    return res.status(200).json(new ApiResponse(200, Profile, "User profile details fetched successfully"))
})
...
export const changePassword = asynchandler(async(req,res,next)=>{
    ...
    if(oldPassword==newPassword && newPassword==confirmPassword){
        return next(new ApiError(401, "New password must be different from the current password"))
    }
    ...
    if(newPassword!==confirmPassword){
        return next(new ApiError(401, "new password and confirm password does not match"))
    }
...
export const getALLUserDetails = asynchandler(async(req,res,next)=>{
    ...
    return res.status(200).json(new ApiResponse(200, userList, "AllUser is fetched successfully"));
...
export const getSingleUserDetails = asynchandler(async(req,res,next)=>{
    ...
    if(!user){
        return next(new ApiError(400, "Details you gived not match user not exist"))
    }
...
export const chaneUserRole = asynchandler(async(req, res, next)=>{
    ...
    const user = await User.findByIdAndUpdate(id, options, {new:true, runValidators:true});

    if(!user){
        return next(new ApiError(400, "user with the given id  not found"))
    }

    return res.status(200).json(new ApiResponse(200, user, "user role is updated successfully"))
})
...
export const deleteUser = asynchandler(async(req,res,next)=>{
    ...
    return res.status(200).json(new ApiResponse(200, [],"User deleted successully"))
})
...
export const createreview = asynchandler(async(req,res,next)=>{
    ...
    let product = await Product.findOneAndUpdate({_id:productid, "reviews.user":req.user.id}, {
        $set:{
    ...
    if (product.reviews.length > 0) {
          avgRating = product.reviews.reduce((acc, item) => acc + item. rating, 0) / product.reviews.length;
    }
...
export const deleteReview = asynchandler(async(req,res,next)=>{
    ...
    ratings = numOfReviews>0? product.reviews.reduce((acc, review)=> acc+review.rating, 0 )/numOfReviews:0;
    ...
    return res.status(200).json(new ApiResponse(200, 4, "product review deleted successfully"))
